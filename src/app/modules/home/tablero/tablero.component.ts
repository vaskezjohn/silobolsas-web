  ...
  ngAfterViewInit(): void {
    this.campoService.CampoList(this.storageService.getCurrentUser().productoresID).toPromise().then((response: any) => {
      response.forEach((item: any) => {
        console.log("ver",item.silobolsas);

        if (item.silobolsas != '')
          this.campos.push(new Campo(item.descripcion, item.calle, item.altura, item.telefono, item.mail, item.ID,
            item.localidadesID, item.productoresID, item.localidades, item.silobolsas));
